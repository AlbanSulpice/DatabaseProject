<template>
    ...
    <p id="navigationComponent">
      ...
      <table v-if="action === 'list'" class="table table-striped table-bordered table-hover"> 
        ...
        <tr v-for="p of sale" v-bind:key="p.sale_id">
          ...
          <td><input type="button" value="DELETE" @click="sendDeleteRequest(p.sale_id)" /></td>
        </tr>
      </table>
    </main>
    <footer id="footer">
  ...
  */
  ...
  async sendDeleteRequest(saleId) {
      try {
        alert("DELETING... " + saleId);
        let response = await this.$http.get("http://localhost:9000/salesapi/del/" + saleId);
    ...
    async sendEditRequest() {
      try {
        alert("EDITING... " + this.oneSale.sale_id);
        let response = await this.$http.post(
              "http://localhost:9000/salesapi/update/" + this.oneSale.sale_id, this.oneSale);
